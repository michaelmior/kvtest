...
To understand how link capacity affects the perform. We also tested our micro benchmark by varying the link capacity. The result is shown in figure~\ref{fig:varying_bd}. During each of the experiment, the system parameters, e.g. the minimum bandwidth guarantee for each server (server 1 will always have min-guarantee off 80\% of the total link capacity), are fixed and we only varied the link capacity. We see that our system can adaptively work with different link capacities and our system is uniformly performing better than the baseline system without scheduling. The next promising step is to increase link capacity at larger steps and study how system will react and try to learn if we need to adjust system adaptivity to the link capacity.


\begin{figure}
    \centering
    \includegraphics[scale=0.47]{figures/simple_topo_varying_bd.png}
